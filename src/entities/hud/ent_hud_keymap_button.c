...
static int **create_hud_km_button(int size)
{
    ...
    int *keymap, dg_scene_t *scene)
{
    ...
    int **map = create_hud_km_button(size);
    void *idata[5] = {entity, &pos, &size, keymap, scene};
    ...
    dg_entity_add_component(entity, cpt_script(&scp_hud_keymap_button_init,
        &scp_hud_keymap_button_loop, &scp_hud_keymap_button_end, idata));
    return entity;
}